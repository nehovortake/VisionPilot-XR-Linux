...
# vp_runtime removed; use local paths only
...
try:
    from qt_read_sign import PerceptronSpeedReader
except Exception:
...
class SpeedWeatherPanel(QFrame):
    ...
    def __init__(self, parent=None, sign_icon_dir_path=None):
        ...
        # where sign icons live (local project folder, no vp_runtime)
        base_dir = Path(__file__).resolve().parent
        default_dir = base_dir / "gui_assets" / "signstocluster"
...
class BackgroundWindow(QMainWindow):
    ...
    def _on_elm_speed(self, speed_kmh: int):
        try:
            v = int(speed_kmh)
            if v < 0:
                v = 0
            self._vehicle_speed = v
            if set_vehicle_speed is not None:
                set_vehicle_speed(v)
        except Exception as e:
            print(f"[GUI] ELM callback error: {e}")

    def _start_elm_speed_reader(self):
        if self._elm_reader is not None:
            return
        if ELM327SpeedReader is None:
            print("[GUI] ELM327SpeedReader import failed")
            return
        try:
            self._elm_reader = ELM327SpeedReader(
                port=self._elm_port,
                baudrate=self._elm_baud,
                callback=self._on_elm_speed,
            )
            self._elm_reader.start()
            print(f"[GUI] ELM327 started on {self._elm_port} @ {self._elm_baud}")
        except Exception as e:
            self._elm_reader = None
            print(f"[GUI] ELM327 start failed: {e}")

    def _stop_elm_speed_reader(self):
        if self._elm_reader is None:
            return
        try:
            self._elm_reader.stop()
            print("[GUI] ELM327 stopped")
        except Exception as e:
            print(f"[GUI] ELM327 stop error: {e}")
        finally:
            self._elm_reader = None
    ...
    def _setup_shortcuts(self):
        ...
        # --- VEHICLE SPEED (manual) ---
        #self._sc_up = mk("Up", self._speed_up)  # +1 km/h
        #self._sc_down = mk("Down", self._speed_down)  # -1 km/h
    ...
    def _on_frame(self):
        ...
        if psutil is not None:
            try:
                ...
                self.perf_panel.set_cpu_percent(cpu_pct)
            except Exception:
                pass
        ...
        if pynvml is not None and _nvml_ok:
            try:
                ...
                gpu_pct = float(getattr(util, "gpu", 0.0))
                gpu_mem_mb = mem.used / (1024.0 * 1024.0)
            except Exception:
                pass

        try:
            self.perf_panel.set_gpu_percent(gpu_pct)
        except Exception:
            pass
        try:
            self.perf_panel.set_vram_mb(gpu_mem_mb)
        except Exception:
            pass

        try:
            from image_processing import log_system_metrics
            log_system_metrics(
                fps=float(self._fps_real_text) if self._fps_real_text not in (None, "--") else 0.0,
        ...
        if getattr(self, "_sw_enabled", False) and hasattr(self, "sw_panel") and self.sw_panel is not None:
            try:
                ...
                # --- MAIN SPEED value = vehicle speed (Up/Down), starts at 50 ---
