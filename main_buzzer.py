...
try:
    ...
    print(f"[INIT] buzzer import FAILED: {e}")
    buzzer = None
...
class State:
    def __init__(self):
        ...
        self.use_simulated_vehicle_speed = True
        self.sim_max_speed = 130.0
        self.sim_accel_kmh_per_s = 8.0
        self.sim_decel_kmh_per_s = 35.0
        self.sim_target_speed = 130.0
...
def on_vehicle_speed(speed):
    if state.use_simulated_vehicle_speed:
        return
...
def set_buzzer(active: bool):
    if not state.buzzer_enabled or buzzer is None:
        return
...
def update_simulated_vehicle_speed():
    if not state.use_simulated_vehicle_speed:
        return

    now = time.perf_counter()
    if state.sim_last_update is None:
        state.sim_last_update = now
        return

    dt = max(0.0, now - state.sim_last_update)
    state.sim_last_update = now

    # If a sign is currently visible, its value becomes the active speed limit.
    # If no sign is visible, the simulated vehicle accelerates back to 130 km/h.
    if state.sign_detected_status and state.detected_sign is not None:
        try:
            target = float(state.detected_sign)
        except Exception:
            target = state.sim_max_speed
    else:
        target = state.sim_max_speed

    target = max(0.0, min(float(state.sim_max_speed), target))
    state.sim_target_speed = target

    current = float(state.vehicle_speed)
    if current < target:
        current += state.sim_accel_kmh_per_s * dt
        if current > target:
            current = target
    elif current > target:
        current -= state.sim_decel_kmh_per_s * dt
        if current < target:
            current = target

    state.vehicle_speed = int(round(max(0.0, min(state.sim_max_speed, current))))
    update_overspeed_alert()
...
def process_frame():
    ...
    try:
        ...
        update_simulated_vehicle_speed()
        update_overspeed_alert()
...
def display_status():
    ...
    status = (
        ...
        f"TG:{int(state.sim_target_speed):>3} "
    ...
    if state.tk_status is not None:
        try:
            gui_status = (
                ...
                f"Target: {int(state.sim_target_speed)} km/h | "
...
def run():
    ...
    state.sim_last_update = time.perf_counter()
    init_tk()
...
def shutdown():
    ...
    if state.buzzer_enabled and buzzer is not None:
        try:
            buzzer.cleanup()
        except Exception as e:
            print(f"[MAIN] Buzzer cleanup warning: {e}")
