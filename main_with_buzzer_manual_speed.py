...
GPIO = None
BUZZER_PIN = 33  # BOARD pin numbering
...
try:
    ...
    print(f"[INIT] BUZZER GPIO: OK (pin {BUZZER_PIN})")
except Exception as e:
...
class State:
    def __init__(self):
        ...
        # Manual vehicle speed input + buzzer
        self.vehicle_speed_lock = threading.Lock()
        self.manual_speed_enabled = True
        self.speed_input_thread = None
        self.speed_input_stop = threading.Event()
...
def on_vehicle_speed(speed):
    # External ELM callback kept for compatibility, but manual terminal input has priority.
    if state.manual_speed_enabled:
        return
    try:
        with state.vehicle_speed_lock:
...
def get_vehicle_speed():
    with state.vehicle_speed_lock:
        return int(state.vehicle_speed)


def set_vehicle_speed(value: int):
    with state.vehicle_speed_lock:
        state.vehicle_speed = int(value)
...
def update_overspeed_alert():
    ...
    try:
        if detected is not None:
            overspeed = int(get_vehicle_speed()) > int(float(detected))
    except Exception:
        overspeed = False
...
def speed_input_worker():
    print("\n[MAIN] Manual vehicle speed input enabled.")
    print("[MAIN] Zadaj rýchlosť do terminálu a stlač Enter. Hodnota ostane nastavená, kým nezadáš inú.")
    print("[MAIN] Príklady: 0, 50, 90, 130\n")

    while not state.speed_input_stop.is_set():
        try:
            raw = input("\nZadaj rýchlosť vozidla (km/h): ").strip()
        except EOFError:
            break
        except KeyboardInterrupt:
            break

        if state.speed_input_stop.is_set():
            break
        ...
        try:
            ...
            set_vehicle_speed(value)
            update_overspeed_alert()
            print(f"[MAIN] Nastavená rýchlosť vozidla: {value} km/h")
        except ValueError:
            print("[MAIN] Neplatná hodnota. Zadaj celé číslo, napr. 50.")
...
def init_all():
    ...
    if state.manual_speed_enabled:
        print("[MAIN] [INFO] ELM327 skipped — vehicle speed is controlled manually from terminal")
    elif ELM327SpeedReader is None:
...
def build_overlay_frame(frame_bgr: np.ndarray) -> np.ndarray:
    ...
    cv2.putText(frame, f"Vehicle: {get_vehicle_speed()} km/h", (20, 40),
...
def run():
    ...
    if state.manual_speed_enabled:
        state.speed_input_stop.clear()
        state.speed_input_thread = threading.Thread(target=speed_input_worker, daemon=True)
        state.speed_input_thread.start()
...
def main():
    ...
    print("[MAIN] ============================================")
    print(f"[MAIN] Manual vehicle speed input: {'ON' if state.manual_speed_enabled else 'OFF'}")
    print(f"[MAIN] Buzzer: {'ON' if state.buzzer_enabled else 'OFF'} | GPIO available: {'YES' if BUZZER_AVAILABLE else 'NO'}")
