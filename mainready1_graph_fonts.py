...
# =========================
# GRAPH STYLE - bigger readable text for thesis/report images
# =========================
GRAPH_DPI = 300
GRAPH_FIGSIZE_WIDE = (16, 9)
GRAPH_FIGSIZE_SQUARE = (12, 12)
GRAPH_TITLE_SIZE = 24
GRAPH_LABEL_SIZE = 20
GRAPH_TICK_SIZE = 16
GRAPH_LEGEND_SIZE = 16
GRAPH_ANNOTATION_SIZE = 18
...
def apply_graph_style():
    if plt is None:
        return
    plt.rcParams.update({
        "font.size": GRAPH_LABEL_SIZE,
        "axes.titlesize": GRAPH_TITLE_SIZE,
        "axes.labelsize": GRAPH_LABEL_SIZE,
        "xtick.labelsize": GRAPH_TICK_SIZE,
        "ytick.labelsize": GRAPH_TICK_SIZE,
        "legend.fontsize": GRAPH_LEGEND_SIZE,
        "figure.titlesize": GRAPH_TITLE_SIZE,
        "savefig.dpi": GRAPH_DPI,
        "figure.dpi": GRAPH_DPI,
    })

def finish_graph(ax=None, legend=False):
    if ax is None:
        ax = plt.gca()
    ax.tick_params(axis="both", which="major", labelsize=GRAPH_TICK_SIZE)
    ax.xaxis.label.set_size(GRAPH_LABEL_SIZE)
    ax.yaxis.label.set_size(GRAPH_LABEL_SIZE)
    ax.title.set_size(GRAPH_TITLE_SIZE)
    if legend:
        ax.legend(fontsize=GRAPH_LEGEND_SIZE, loc="best", frameon=True)
...
def shutdown():
    ...
    if plt is not None:
        ...
        def _save_current_plot(out_path: Path):
            ...
            plt.savefig(str(out_path), dpi=GRAPH_DPI, bbox_inches="tight")
            plt.close()
            print(f"[MAIN] Graph saved: {out_path}")

        try:
            if len(state.cpu_samples) > 0:
                plt.figure(figsize=GRAPH_FIGSIZE_WIDE)
                plt.plot(state.timestamps, state.cpu_samples, linewidth=GRAPH_LINE_WIDTH)
                plt.xlabel("Čas (s)")
                ...
                plt.text(0.5, 0.95, f"Priemer: {avg:.1f}%", transform=plt.gca().transAxes, ha="center", fontsize=GRAPH_ANNOTATION_SIZE, fontweight="bold")
                finish_graph()
            ...
            if len(state.graph_timestamps) > 0 and len(state.output_fps_samples) > 0:
                ...
                plt.plot(x, cam, linewidth=GRAPH_LINE_WIDTH, label=f"FPS kamery (priemer {avg_cam:.1f})")
                plt.plot(x, out, linewidth=GRAPH_LINE_WIDTH, label=f"FPS po spracovaní (priemer {avg_out:.1f})")
                plt.xlabel("Čas (s)")
                plt.ylabel("FPS")
                plt.title("Porovnanie FPS kamery a výstupných FPS")
                plt.grid(True, alpha=0.3)
                finish_graph(legend=True)
            ...
            if len(state.graph_timestamps) > 0 and len(state.proc_ms_samples) > 0 and len(state.total_ms_samples) > 0:
                ...
                plt.plot(x, total, linewidth=GRAPH_LINE_WIDTH, label=f"Celkový čas (priemer {avg_total:.1f} ms)")
                plt.plot(x, proc, linewidth=GRAPH_LINE_WIDTH, label=f"Čas spracovania (priemer {avg_proc:.1f} ms)")
                plt.xlabel("Čas (s)")
            ...
            if len(state.graph_timestamps) > 0:
                ...
                for label, arr in series:
                    ...
                    if len(arr2) > 0:
                        ...
                        plt.plot(x, arr2, linewidth=GRAPH_LINE_WIDTH, label=f"{label} (priemer {avg_stage:.1f} ms)")
                        anything = True
                if anything:
                    plt.xlabel("Čas (s)")
                    plt.ylabel("Čas (ms)")
            ...
            if len(state.detection_events) > 0:
                ...
                plt.figure(figsize=GRAPH_FIGSIZE_SQUARE)
                plt.pie(
                    values,
                    labels=labels,
                    autopct="%1.1f%%",
                    startangle=90,
                    textprops={"fontsize": GRAPH_TICK_SIZE, "fontweight": "bold"},
                )
                plt.title("Rozdelenie detekcií", fontsize=GRAPH_TITLE_SIZE, fontweight="bold")
