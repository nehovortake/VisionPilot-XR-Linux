...
class State:
    def __init__(self):
        ...
        self.tk_speed_frame = None
        self.tk_speed_label = None
        self.tk_speed_entry = None
        self.tk_speed_button = None
...
def on_vehicle_speed(speed):
    # When manual speed is enabled, ignore ELM updates.
...
def apply_manual_speed_from_entry(event=None):
    if state.tk_speed_entry is None:
        return

    raw = state.tk_speed_entry.get().strip()
    if raw == "":
        return
    ...
    try:
        ...
        update_overspeed_alert()
        print(f"\n[MAIN] Nastavená rýchlosť vozidla: {value} km/h")
        try:
            state.tk_speed_entry.delete(0, tk.END)
            state.tk_speed_entry.insert(0, str(value))
            state.tk_speed_entry.select_range(0, tk.END)
        except Exception:
            pass
    except ValueError:
        print("\n[MAIN] Neplatná hodnota rýchlosti. Zadaj napr. 0, 50, 90 alebo 130.")
...
def init_all():
    ...
    if state.manual_speed_enabled:
        print("[MAIN] [INFO] ELM327 skipped — vehicle speed is controlled manually in GUI")
...
def build_overlay_frame(frame_bgr: np.ndarray) -> np.ndarray:
    ...
    cv2.putText(frame, f"Alarm: {'ON' if state.overspeed_active else 'OFF'}", (20, 355),
...
def init_tk():
    ...
    speed_frame = tk.Frame(root, bg="#101010")
    speed_frame.pack(pady=(0, 8))

    speed_label = tk.Label(
        speed_frame,
        text="Nastav rýchlosť:",
        font=("Arial", 11, "bold"),
        fg="white",
        bg="#101010"
    )
    ...
    speed_entry.insert(0, str(state.vehicle_speed))
    speed_entry.pack(side="left", padx=(0, 8))
    speed_entry.bind("<Return>", apply_manual_speed_from_entry)

    speed_button = tk.Button(
        speed_frame,
        text="Nastaviť",
        font=("Arial", 10, "bold"),
        command=apply_manual_speed_from_entry
    )
    ...
    state.tk_speed_frame = speed_frame
    state.tk_speed_label = speed_label
    state.tk_speed_entry = speed_entry
    state.tk_speed_button = speed_button
