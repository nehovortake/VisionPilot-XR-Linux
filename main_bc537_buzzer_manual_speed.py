...
GPIO = None
BUZZER_PIN = 33  # BOARD pin; this pin drives BC537 base through 1k resistor
...
def on_vehicle_speed(speed):
    # Keep compatibility with ELM callback, but manual speed has priority.
...
def init_all():
    ...
    if state.manual_speed_enabled:
        print("[MAIN] [INFO] ELM327 skipped — vehicle speed is set manually in GUI")
...
def build_overlay_frame(frame_bgr: np.ndarray) -> np.ndarray:
    ...
    cv2.putText(frame, f"CPU: {state.last_cpu:5.1f}%", (20, 315),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    cv2.putText(frame, f"Alarm: {'ON' if state.overspeed_active else 'OFF'}", (20, 350),
